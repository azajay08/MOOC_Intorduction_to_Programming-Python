...
with open(exam_points) as exa:
	for line in exa:
		...
		exa_count = 0
		if l_split[0] in pupil:
			exe_points = (pupil[l_split[0]]['exec_nbr'] / 40) * 100 // 10
			# print(int(exe_points))
			for num in l_split[1:]:
				exa_count += int(num)
			pupil[l_split[0]]["exm_pts"] = get_grade(exa_count + int(exe_points))

for scores in pupil:
	print(f"{pupil[scores]['first']} {pupil[scores]['last']} {pupil[scores]['exm_pts']}")
