...
def accepted(attempts: list):
	return filter(lambda s : s.grade > 1, attempts)

def attempts_with_grade(attempts: list, grade: int):
	return filter(lambda s : s.grade == grade, attempts)

def passed_students(attempts: list, course: str):
	return sorted(list(map(lambda s : s.student_name, filter(lambda g : g.grade > 0, filter(lambda c : c.course_name == course, attempts)))))
